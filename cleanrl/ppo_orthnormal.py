...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.bias5 = nn.Parameter(torch.zeros(64), requires_grad=True)
        self.bias6 = nn.Parameter(torch.zeros(1), requires_grad=True)

    def get_value(self, x):
        ##EDIT#기본 ppo 알고리즘대로 value 추측
        x = F.linear(x, (self.fixed_weights1 * self.scale1.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias1)
        x = F.tanh(x)
        x = F.linear(x, (self.fixed_weights2 * self.scale2.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias2)
        x = F.tanh(x)
        x = F.linear(x, (self.fixed_weights3 * self.scale3.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias3)
        return x

    def get_action_and_value(self, x, action=None):
        ##EDIT# 기본 ppo 알고리즘대로 action,valuevalue 추측
        logits = F.linear(x, (self.fixed_weights4 * self.scale4.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias4)
        logits = F.tanh(logits)
        logits = F.linear(logits, (self.fixed_weights5 * self.scale5.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias5)
        logits = F.tanh(logits)
        logits = F.linear(logits, (self.fixed_weights6 * self.scale6.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias6)
