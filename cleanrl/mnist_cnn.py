...
class LCRM(nn.Module):
    ...
    def __init__(self):
        ...
        self.fixed_weights1 = nn.Parameter(make_random(28*28,1000,100,100),requires_grad=False)
        self.scale1 = nn.Parameter(torch.randn(100), requires_grad=True)
        self.fixed_weights2 = nn.Parameter(make_random(1000,1000,100,100),requires_grad=False)
        self.scale2 = nn.Parameter(torch.randn(100), requires_grad=True)
        self.fixed_weights3 = nn.Parameter(zero_one_random(1000,500,100,100),requires_grad=False)
        self.scale3 = nn.Parameter(torch.randn(100), requires_grad=True)
        self.fixed_weights4 = nn.Parameter(zero_one_random(500,200,100,100),requires_grad=False)
        self.scale4 = nn.Parameter(torch.randn(100), requires_grad=True)
        self.fixed_weights5 = nn.Parameter(zero_one_random(200,10,100,100),requires_grad=False)
        self.scale5 = nn.Parameter(torch.randn(100), requires_grad=True)
        ...
        self.bias_scale1 = nn.Parameter(torch.zeros( 1000), requires_grad=False)
        self.bias_scale2 = nn.Parameter(torch.zeros( 1000), requires_grad=False)
        self.bias_scale3 = nn.Parameter(torch.zeros( 500), requires_grad=False)
        self.bias_scale4 = nn.Parameter(torch.zeros( 200), requires_grad=False)
        self.bias_scale5 = nn.Parameter(torch.zeros( 10), requires_grad=False)

        # self.bias_scale4= nn.Parameter(torch.zeros( 10), requires_grad=True)
        init.normal_(self.scale1, mean=0, std=0.1)
        init.normal_(self.scale2, mean=0, std=0.1)
        init.normal_(self.scale3, mean=0, std=0.1)
        init.normal_(self.scale4, mean=0, std=0.1)
        init.normal_(self.scale5, mean=0, std=0.1)
    ...
    def forward(self,  x):
        ...
        x = F.relu(x)
        x = F.linear(x, (self.fixed_weights4 * self.scale4.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias_scale4)
        x = F.relu(x) # sigmoid(x)
        x = F.linear(x, (self.fixed_weights5 * self.scale5.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias_scale5)
        # x = F.relu(x) # sigmoid(x)
        # x = F.linear(x, (self.fixed_weights6 * self.scale6.unsqueeze(-1).unsqueeze(-1)).sum(dim=0).T,bias=self.bias_scale6)
