...
"""
    ...
    for global_step in range(args.total_timesteps):
        ...
        if random.random() < epsilon:
            ...
            q_value1 = em1.Q_estimate(obs,args.number_neighbor)
            q_value2 = em2.Q_estimate(obs,args.number_neighbor)
            action_Q[0]=q_value1
            action_Q[1]=q_value2 
        ...
        if "final_info" in infos:
            ...
            for i in range(episode_pos-1,-1,-1):
                ...
                if episode_actions[i]==0:
                    update_Q=max(em1.Q_estimate(episode_observations[i],args.number_neighbor),return_value)
                    em1.add(episode_observations[i],update_Q)
                else:
                    update_Q=max(em2.Q_estimate(episode_observations[i],args.number_neighbor),return_value)
                    em2.add(episode_observations[i],update_Q)
