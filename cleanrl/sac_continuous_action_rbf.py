...
"""
    ...
    r_tsne = ReplayBuffer_get(
        args.buffer_size_tsne,
        envs.single_observation_space,
        envs.single_action_space,
        device,
        handle_timeout_termination=False,
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            actor = Actor(envs,v_trans.T).to(device) 
    ...
    colors = np.linspace(0, 1, num_points // 5000)  # 각 10,000개마다 고유한 색상 값 할당
    labels = np.repeat(colors, 5000)
    ...
    plt.savefig(f'tsne_sac_cotinuous_action_svd_scailing{args.seed}.png')
    envs.close()
    writer.close()
